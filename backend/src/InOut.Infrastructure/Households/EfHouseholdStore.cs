...
public sealed class EfHouseholdStore(InOutDbContext dbContext) : IHouseholdStore
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        AddAudit(
            household.Id,
            userId,
            PersistenceVocabulary.AuditActions.HouseholdCreated,
            PersistenceVocabulary.EntityTypes.Household,
            household.Id);
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (role != HouseholdRole.Owner)
        {
            throw new HouseholdRuleException(
                HouseholdErrorCodes.OwnerRequired,
                "Only a household owner can create an invitation.");
        }
        ...
        {
            throw new HouseholdRuleException(
                HouseholdErrorCodes.HouseholdFull,
        ...
        AddAudit(
            householdId,
            userId,
            PersistenceVocabulary.AuditActions.InvitationCreated,
            PersistenceVocabulary.EntityTypes.HouseholdInvitation,
            invitation.Id);
        await dbContext.SaveChangesAsync(cancellationToken);
        await transaction.CommitAsync(cancellationToken);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (alreadyMember)
        {
            throw new HouseholdRuleException(HouseholdErrorCodes.AlreadyMember, "User is already a household member.");
        }
        ...
        if (memberCount >= HouseholdRules.MaximumMembers)
        {
            throw new HouseholdRuleException(HouseholdErrorCodes.HouseholdFull, "Household already has two members.");
        }
        ...
        AddAudit(
            ...
            userId,
            PersistenceVocabulary.AuditActions.InvitationAccepted,
            PersistenceVocabulary.EntityTypes.HouseholdInvitation,
            invitation.Id);
